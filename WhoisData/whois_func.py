...
from Setting.global_resource import Resource  # 全局资源

Resource.global_object_init()
Static.log_init()
func_log = Static.LOGGER


def com_manage(data, domain_whois):
    """存在二级服务器的 提取函数"""
    if domain_whois['sec_whois_server'] != '':
        sec_func_name = Resource.WhoisFunc.get_whois_func(domain_whois['sec_whois_server'])
        if sec_func_name is not None:  # 存在专用的二级服务器处理函数
            domain_whois = eval('{func}(data, domain_whois)'.format(func=sec_func_name))
    else:
