...
class GetWhoisInfo:
    ...
    def get(self):
        """获取数据"""
        data = self.__connect()
        if not GetWhoisInfo.__is_error(data):  # 如果数据没有错误,则直接返回
            return data
        # 处理异常类型
    ...
    def __connect(self):
        ...
        try:
            ...
            if str(err).find("timed out") != -1 or \
                ...
                return "ERROR -1"
            elif str(err).find("Temporary failure in name resolution") != -1 or \
                    str(err).find("cannot connect to identd on the client") != -1 or \
                    str(err).find("unreachable") != -1:
                self.tcpCliSock.close()
                return "ERROR -2"
            else:
                self.tcpCliSock.close()
